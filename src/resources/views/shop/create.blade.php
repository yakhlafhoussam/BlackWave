    ...
    <div class="rounded-2xl border border-white/10 bg-black/50 backdrop-blur-sm p-6 md:p-8">
        <form method="POST" action="#" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                @enderror
                <p class="text-xs text-gray-500">Choose a clear and descriptive title for your product (max 100 characters)</p>
            </div>
                ...
                <div class="relative">
                    ...
                    >
                        ...
                        @foreach($categories ?? [] as $category)
                            <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }} class="bg-gray-900">
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-2" x-data="{ imagePreview: null, fileName: '' }">
                ...
                <div class="relative">
                    ...
                    />
                    <div class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/20 bg-white/5 p-8 cursor-pointer hover:border-blue-500/50 transition-colors">
                        ...
                        <template x-if="imagePreview">
                            <div class="relative w-full">
                                <div class="absolute top-2 right-2 z-20">
                                    <button type="button" @click="imagePreview = null; document.getElementById('image').value = ''; fileName = ''" class="bg-black/80 rounded-full p-1.5 hover:bg-red-500 transition-colors">
            ...
            <div class="space-y-2">
                ...
                <textarea
                    ...
                    class="w-full rounded-xl border bg-white/5 px-4 py-3 text-white placeholder-gray-500 transition-all duration-200 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 @error('description') border-red-500 @else border-white/10 @enderror"
                >{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-sm text-red-400">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-500">Provide a detailed description of your product (minimum 50 characters)</p>
            </div>
            ...
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        <div class="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                            @if(auth()->user()->profile_image)
                                <img src="{{ asset('storage/' . auth()->user()->profile_image) }}" class="h-full w-full rounded-full object-cover">
                            @else
                                <span class="text-white text-xs font-bold">{{ substr(auth()->user()->username, 0, 2) }}</span>
                            @endif
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <p class="text-gray-400 text-sm mt-1" id="preview-description">Product description preview...</p>
                    </div>
            ...
            <div class="flex gap-4 pt-4">
                <a href="{{ route('marketplace') }}" class="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white/10 border border-white/20 text-white font-semibold rounded-xl transition-all duration-200 hover:bg-white/20">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                    Cancel
                </a>
                <button type="submit" class="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl transition-all duration-200 hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    if (titleInput && previewTitle) {
        titleInput.addEventListener('input', function() {
            previewTitle.textContent = this.value.trim() === '' ? 'Your product title will appear here' : this.value;
        });
    }
    ...
    if (descriptionInput && previewDescription) {
        descriptionInput.addEventListener('input', function() {
            ...
            previewDescription.textContent = text === '' ? 'Product description preview...' : text.substring(0, 100) + (text.length > 100 ? '...' : '');
        });
    }
